#!/bin/bash
# script to deactive user using salesforce cli
...
# no inputs or more than 2 and we just fail out
  ...
  echo "$0 username@to.deactivate [org-alias-or-user]"
  echo ""
  ...
  if [ "$#" = "1" ]; then
    ...
    echo ""
    echo "Deactivation will happen in the org for the default project user. Enter 'y' to proceed."
...
echo "RUNNING COMMAND: ${command} ${sobject} ${whereflag} ${values} ${adminuser}"
  
#run the command and pass through stdout. 
$command $sobject $whereflag $values $adminuser

echo "Done."


# sfdx force:data:record:update -w [where record matches field value] -s User -u [my alias] -v IsActive=false