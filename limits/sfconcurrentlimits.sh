...
#capture stdout
dxoutput=$(date ; sfdx force:limits:api:display $adminuser | grep -v 'Daily' | grep -v 'Package2' | grep -v 'Hourly' | grep -v 'SingleEmail'  | grep -v 'MassEmail' | grep -v 'Storage' | grep -v 'PermissionSets')
